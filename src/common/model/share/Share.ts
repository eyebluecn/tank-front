...
export default class Share extends BaseEntity {
  ...
  getIcon() {
    if (this.shareType === ShareType.MIX) {
      ...
      return FileUtil.getIcon(this.name as any, this.shareType === ShareType.DIRECTORY)
    }
  }
  ...
  downloadZip(rootUuid: string, puuid?: string) {
    window.open(`${EnvUtil.currentHost()}${Share.URL_ZIP}?shareUuid=${this.uuid}&code=${this.code}&puuid=${puuid}&rootUuid=${rootUuid}`)
  }

  //创建一个分享matterUuids要求为数组，expireTime要求为时间对象
  httpCreate(matterUuids: string, successCallback?: () => any, errorCallback?: () => any) {
    let that = this
  ...
  httpBrowse(puuid: string, rootUuid: string, successCallback?: () => any, errorCallback?: () => any) {
    let that = this
