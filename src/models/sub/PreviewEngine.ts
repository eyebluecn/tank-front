...
export interface IPreviewEngine {
  /**
   * 此预览引擎的url
   ...
   * 本站预览 or 新标签打开
   */
  previewInSite: boolean
}
export default class PreviewEngine implements IPreviewEngine {
  url = ''
  extensions = ''
  previewInSite = true

  constructor(obj?: IPreviewEngine) {
    obj && ObjectUtil.extend(this, obj)
  }

  //提供几个默认的预览引擎。
  static defaultPreviewEngines(): PreviewEngine[] {
    const defaultEngines: PreviewEngine[] = []

    //添加office默认预览引擎，使用微软开放接口
    const officeEngine: PreviewEngine = new PreviewEngine()
    ...
    //添加全局预览引擎，使用浏览器预览能力。
    const globalEngine: PreviewEngine = new PreviewEngine()
